with calendar_bounds as (
    select
        least(
            (select min(order_date) from {{ ref('stg_b2_stg__sales') }}),
            (select min(cast(order_datetime_ts as date)) from {{ ref('stg_b2_stg__sales_orders') }})
        ) as min_date,
        greatest(
            (select max(order_date) from {{ ref('stg_b2_stg__sales') }}),
            (select max(cast(order_datetime_ts as date)) from {{ ref('stg_b2_stg__sales_orders') }})
        ) as max_date
),
calendar as (
